...
/* ajax id 중복체크	 */
	...
	function checkform() {
		var id = document.addform.user_id.value;
		var regx = /^[a-zA-Z0-9]*$/;
		var chk1 = /\d/;
		var kor = /^[가-힣]*$/;
		...
		// 아이디 입력여부 검사
		if (addform.user_id.value == "") {
			alert("아이디를 입력해주세요.")
			addform.user_id.focus()
			return false;
		}
		...
		if (document.addform.password.value != document.addform.password2.value) {
			alert("비밀번호가 일치하지 않습니다")
			document.addform.password2.value = ""
		...
		var tel = document.addform.phone.value;
			...
			<div id="Add" class="Tabs">
				<div id="add_search_border">
					...
					<ul>
						...
						<li class="textstyle">
							<input type="button" id="ajaxid" value="ID 중복체크">
							<input type="hidden" id="ajaxid_1" value="N">
						</li>
					</ul>
					&nbsp;&nbsp;&nbsp;<span id="idspan"></span>
					<ul>
					...
					<ul id="business_display2" style="display: none">
						...
						<li class="textstyle">
							...
							<input type="button" onclick="searchaddr()" value="우편번호 찾기" id="kk_searchaddr">
						</li>
						<li class="textstyle">
					...
					<ul id="btnstyle">
						...
						<li>
							<input type="reset" id="reset" name="resetbtn" value="취소" onclick="location.href='<%=request.getContextPath()%>/kat_main.do'">
						</li>
					</ul>
				</div>
			</div>
